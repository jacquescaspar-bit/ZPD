...
const SubmissionConfirmation = ({ onClose, referralCode: propReferralCode }: SubmissionConfirmationProps) => {
  const [copied, setCopied] = useState(false);
  const [generatedCode] = useState(() => 
    propReferralCode ?? `REF-${Math.random().toString(36).toUpperCase().slice(2, 8)}`
  );
